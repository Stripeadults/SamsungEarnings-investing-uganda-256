...
const PACKAGES_MAP: Record<string, number> = {
  ...
  'galaxy-s24': 160000, 'galaxy-s24-plus': 280200,
  'galaxy-s24-ultra': 450000, 'galaxy-z-flip6': 500000, 'galaxy-z-fold6': 700000,
};
...
Deno.serve(async (req: Request) => {
  ...
  let from = 0;
  while (true) {
    const { data } = await supabase.from('samsung_products').select('id, user_id, package_id, package_name, daily_income, status, expiry_date, last_income_date, total_income_earned').range(from, from + 999);
