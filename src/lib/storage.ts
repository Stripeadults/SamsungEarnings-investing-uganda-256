...
function dbToUser(r: Record<string, unknown>): User {
  return {
    ...
    registrationBonus: Number(r.registration_bonus ?? 7000), referralCode: r.referral_code as string,
...
export async function createProduct(p: any): Promise<void> {
  ...
  if (error) { console.error('createProduct error', error); throw new Error(error.message); }
}
...
export async function createRecharge(r: Recharge): Promise<void> { 
  ...
  if (error) { console.error('Recharge insert failed:', error); throw new Error(error.message); }
}
...
export async function runDailyIncomeWithStats(): Promise<{ credited: number; total: number }> {
  ...
  for (const product of products) {
    ...
    const lockedIncome = pkg ? pkg.dailyIncome : product.dailyIncome;
