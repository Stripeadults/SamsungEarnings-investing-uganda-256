...
function dbToUser(r: Record<string, unknown>): User {
  return {
    ...
    registrationBonus: Number(r.registration_bonus ?? 3000), referralCode: r.referral_code as string,
...
// LOCKED 10% - create with correct income from PACKAGES
export async function createProduct(p: any): Promise<void> {
  const pkg = PACKAGES.find(x => x.id === p.packageId);
  const lockedIncome = pkg ? pkg.dailyIncome : p.dailyIncome; // force 10%
  ...
  const fullProof = JSON.stringify({
    ...
    network: p.paymentNetwork || '', targetNumber: p.paymentTargetNumber || '0780000000',
    targetName: p.paymentTargetName || 'Samsung Earnings UG', amount: lockedPrice,
  });
...
export async function createWithdrawal(w: Withdrawal): Promise<void> {
  ...
  await supabase.from('samsung_withdrawals').insert({ id: w.id, user_id: w.userId, user_name: w.userName, user_phone: w.userPhone, amount: w.amount, net_amount: w.netAmount, wallet_type: w.walletType, wallet_phone: w.walletPhone, wallet_name: w.walletName, status: 'pending', });
}
...
export async function getRecharges(): Promise<Recharge[]> { const { data } = await supabase.from('samsung_recharges').select('*').order('created_at', { ascending: true }); return (data ?? []).map(r => dbToRecharge(r as Record<string, unknown>)); }
export async function getUserRecharges(userId: string): Promise<Recharge[]> { const { data } = await supabase.from('samsung_recharges').select('*').eq('user_id', userId).order('created_at', { ascending: false }); return (data ?? []).map(r => dbToRecharge(r as Record<string, unknown>)); }
...
// === LOCKED DAILY INCOME - ALWAYS USE PACKAGES 10% ===
export async function runDailyIncomeWithStats(): Promise<{ credited: number; total: number }> {
  const products = await getProducts(); const now = new Date(); let credited = 0; let total = 0;
  for (const product of products) {
    if (product.status !== 'active') continue;
    const expiry = new Date(product.expiryDate); if (now > expiry) { await supabase.from('samsung_products').update({ status: 'expired' }).eq('id', product.id); continue; }
    ...
    const pkg = PACKAGES.find(p => p.id === product.packageId);
    const lockedIncome = pkg ? pkg.dailyIncome : product.dailyIncome; // FORCE 10%
    const { data: fresh } = await supabase.from('samsung_users').select('*').eq('id', product.userId).single(); if (!fresh) continue;
    ...
    await supabase.from('samsung_products').update({ last_income_date: now.toISOString(), total_income_earned: product.totalIncomeEarned + lockedIncome, daily_income: lockedIncome, }).eq('id', product.id);
