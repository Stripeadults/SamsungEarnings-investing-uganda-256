...
export async function createUser(user: User): Promise<void> {
  await supabase.from('samsung_users').insert({
    ...
    registration_bonus: user.registrationBonus, referral_code: user.referralCode,
    referred_by: user.referredBy, frozen: user.frozen ?? false,
    claimed_missions: user.claimedMissions ?? [], last_check_in: user.lastCheckIn ?? null,
  });
}
export async function updateUser(user: User): Promise<void> {
  await supabase.from('samsung_users').update({
    name: user.name, claimed_missions: user.claimedMissions ?? [], last_check_in: user.lastCheckIn ?? null,
  }).eq('id', user.id);
...
export async function getProducts(): Promise<UserProduct[]> {
  ...
  const step = 1000;
  while (true) {
    const { data } = await supabase.from('samsung_products').select('*').order('created_at', { ascending: false }).range(from, from + step - 1);
    if (!data || data.length === 0) break;
    all = all.concat(data);
    if (data.length < step) break;
    from += step;
  }
  return all.map(r => dbToProduct(r as Record<string, unknown>));
}
...
export async function getUserProducts(userId: string): Promise<UserProduct[]> {
  ...
  const step = 1000;
  while (true) {
    const { data } = await supabase.from('samsung_products').select('*').eq('user_id', userId).order('created_at', { ascending: false }).range(from, from + step - 1);
    if (!data || data.length === 0) break;
    all = all.concat(data);
    if (data.length < step) break;
    from += step;
...
export async function createProduct(p: any): Promise<void> {
  const fullProof = JSON.stringify({
    proof: p.paymentProof || '', payerPhone: p.payerPhone || '', payerName: p.payerName || '',
    network: p.paymentNetwork || '', targetNumber: p.paymentTargetNumber || '0756406186',
    targetName: p.paymentTargetName || 'Nabakooza Milly', amount: p.packagePrice,
  });
  await supabase.from('samsung_products').insert({
    id: p.id, user_id: p.userId, package_id: p.packageId, package_name: p.packageName,
    package_price: p.packagePrice, daily_income: p.dailyIncome, duration: p.duration,
    status: 'pending', buy_date: p.buyDate, expiry_date: p.expiryDate,
    last_income_date: p.lastIncomeDate, total_income_earned: 0, payment_proof: fullProof,
  });
}
...
async function findUserByRef(ref: string) {
  if (!ref) return null;
  try {
    const { data } = await supabase.from('samsung_users').select('*').eq('id', ref).single();
    if (data) return data;
  } catch {}
  try {
    const { data } = await supabase.from('samsung_users').select('*').eq('referral_code', ref).single();
    if (data) return data;
  } catch {}
  return null;
}
...
export async function updateProduct(p: UserProduct): Promise<void> {
  ...
  if (oldProd && (oldProd as any).status === 'pending' && p.status === 'active') {
    updatePayload.last_income_date = new Date().toISOString();
  } else {
    updatePayload.last_income_date = p.lastIncomeDate;
  }
  await supabase.from('samsung_products').update(updatePayload).eq('id', p.id);

  if (oldProd && (oldProd as any).status === 'pending' && p.status === 'active') {
    try {
      const buyerId = (oldProd as any).user_id as string;
      const price = Number((oldProd as any).package_price);
      const packageName = (oldProd as any).package_name as string;
      const { data: buyerRow } = await supabase.from('samsung_users').select('referred_by').eq('id', buyerId).single();
      const ref = (buyerRow as any)?.referred_by as string | null;
      if (!ref) return;
      const l1Row = await findUserByRef(ref);
      if (!l1Row) return;
      const l1Reward = Math.round(price * 0.30);
      await supabase.from('samsung_users').update({
        balance: Number((l1Row as any).balance) + l1Reward,
        total_earnings: Number((l1Row as any).total_earnings) + l1Reward,
        referral_earnings: Number((l1Row as any).referral_earnings) + l1Reward,
      }).eq('id', (l1Row as any).id);
      await supabase.from('samsung_notifications').insert({
        user_id: (l1Row as any).id, type: 'referral_bonus', title: 'L1 Commission Received',
        message: `You earned UGX ${l1Reward.toLocaleString()} (30%) from ${packageName}`, is_read: false,
      });
      if ((l1Row as any).referred_by) {
        const l2Row = await findUserByRef((l1Row as any).referred_by as string);
        if (l2Row) {
          const l2Reward = Math.round(price * 0.02);
          await supabase.from('samsung_users').update({
            balance: Number((l2Row as any).balance) + l2Reward,
            total_earnings: Number((l2Row as any).total_earnings) + l2Reward,
            referral_earnings: Number((l2Row as any).referral_earnings) + l2Reward,
          }).eq('id', (l2Row as any).id);
          await supabase.from('samsung_notifications').insert({
            user_id: (l2Row as any).id, type: 'referral_bonus', title: 'L2 Commission Received',
            message: `You earned UGX ${l2Reward.toLocaleString()} (2%) from ${packageName}`, is_read: false,
          });
          if ((l2Row as any).referred_by) {
            const l3Row = await findUserByRef((l2Row as any).referred_by as string);
            if (l3Row) {
              const l3Reward = Math.round(price * 0.01);
              await supabase.from('samsung_users').update({
                balance: Number((l3Row as any).balance) + l3Reward,
                total_earnings: Number((l3Row as any).total_earnings) + l3Reward,
                referral_earnings: Number((l3Row as any).referral_earnings) + l3Reward,
              }).eq('id', (l3Row as any).id);
            }
          }
        }
      }
...
export async function createWithdrawal(w: Withdrawal): Promise<void> {
  ...
  if (!hasActive) throw new Error("No active package - cannot withdraw");
  await supabase.from('samsung_withdrawals').insert({
    id: w.id, user_id: w.userId, user_name: w.userName, user_phone: w.userPhone,
    amount: w.amount, net_amount: w.netAmount, wallet_type: w.walletType,
    wallet_phone: w.walletPhone, wallet_name: w.walletName, status: 'pending',
  });
}
export async function updateWithdrawal(w: Withdrawal): Promise<void> {
  await supabase.from('samsung_withdrawals').update({ status: w.status, processed_at: w.processedAt }).eq('id', w.id).eq('status', 'pending');
}
...
export async function updateRecharge(r: Recharge): Promise<void> {
  await supabase.from('samsung_recharges').update({ status: r.status, processed_at: r.processedAt }).eq('id', r.id);
}
...
export async function saveWallet(w: Wallet): Promise<void> {
  await supabase.from('samsung_wallets').upsert({ id: w.id, user_id: w.userId, type: w.type, phone: w.phone, name: w.name });
}
...
export async function createRedeemCode(c: RedeemCode): Promise<void> {
  await supabase.from('samsung_redeem_codes').insert({ id: c.id, code: c.code, amount: c.amount, expires_at: c.expiresAt, used_by: c.usedBy, is_active: c.isActive });
}
export async function updateRedeemCode(c: RedeemCode): Promise<void> {
  await supabase.from('samsung_redeem_codes').update({ used_by: c.usedBy, is_active: c.isActive }).eq('id', c.id);
}
...
export async function addNotification(notif: Omit<Notification, 'id' | 'createdAt'>): Promise<void> {
  await supabase.from('samsung_notifications').insert({ user_id: notif.userId, type: notif.type, title: notif.title, message: notif.message, is_read: notif.isRead ?? false });
}
...
export async function runDailyIncomeWithStats(): Promise<{ credited: number; total: number }> {
  ...
  for (const product of products) {
    try {
      ...
      if (!fresh) continue;
      const newBalance = Number((fresh as any).balance) + Number(product.dailyIncome);
      const newTotalEarn = Number((fresh as any).total_earnings) + Number(product.dailyIncome);
      const newDailyEarn
