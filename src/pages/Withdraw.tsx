...
export default function Withdraw() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        setHasPackage(!!(data && data.length > 0));
      } catch {
        setHasPackage(false);
      }
    };
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      if (!activeCheck || activeCheck.length === 0) {
        toast.error('No active package - cannot withdraw');
        setHasPackage(false);
        setLoading(false);
        return;
      }
    ...
    return (
      <div className="p-6 pb-20 pt-24 text-center">
        <div className="bg-red-50 border-2 border-red-300 rounded-2xl p-6">
          ...
          <p className="text-sm text-gray-600 mt-3">You have no ACTIVE package. Pending packages cannot withdraw until admin approves payment.</p>
          <button onClick={()=>navigate('/product')} className="bg-blue-600 text-white w-full p-3 rounded-xl font-bold mt-6">Buy Package Now</button>
          <button onClick={()=>navigate('/home')} className="bg-gray-200 text-black w-full p-3 rounded-xl font-bold mt-3">Go Home</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4 pb-20 pt-16">
