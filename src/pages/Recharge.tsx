...
const Recharge = () => {
  ...
  const handleSubmit = async () => {
    if (!senderPhone.trim() ||!senderName.trim() ||!proof.trim()) {
    ...
    try {
      ...
      console.log('Submitting recharge:', recharge);
      const result = await createRecharge(recharge);
      ...
      toast.error(e?.message || 'Submit failed - try again. Check internet.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="app-container min-h-screen bg-gray-50">
      ...
      {step === 2 && (
        <div className="px-4 py-5 space-y-4">
          <div className="rounded-2xl p-5 text-white" style={{ background: 'linear-gradient(135deg, #0a0f2e, #1d4ed8)' }}>
            <div className="text-white/70 text-sm mb-3">Send {formatUGX(parseInt(amount || '0'))} to:</div>
          ...
          <div>
            ...
            <input type="text" value={senderName} onChange={(e) => setSenderName(e.target.value)} placeholder="Name on mobile money" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none text-base bg-white" />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600 mb-2 block">Payment Proof (M-Money SMS) *</label>
            <textarea value={proof} onChange={(e) => setProof(e.target.value)} placeholder="Paste confirmation message here..." className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none text-sm bg-white h-28 resize-none" />
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={handleSubmit} disabled={submitting} className="flex-1 py-4 rounded-xl text-white font-bold disabled:opacity-60 disabled:bg-gray-400" style={{ background: submitting? '#9ca3af' : 'linear-gradient(135deg, #1d4ed8, #3b82f6)' }}>
      ...
      {step === 3 && (
        <div className="px-4 py-8 flex flex-col items-center">
          <div className="w-20 h-20 rounded-full bg-green-100 flex items-center justify-center mb-4"><span className="text-4xl">✅</span></div>
          <h2 className="text-gray-800 font-bold text-xl mb-2">Recharge Submitted!</h2>
          <p className="text-gray-500 text-sm text-center mb-6 leading-relaxed">
            Your recharge of <span className="font-bold text-blue-600">{formatUGX(parseInt(amount || '0'))}</span> has been submitted. Admin will approve shortly.
          </p>
          <div className="w-full bg-white rounded-2xl p-5 shadow-sm mb-4 space-y-3">
            <div className="flex justify-between text-sm"><span className="text-gray-400">Amount</span><span className="text-gray-800 font-bold">{formatUGX(parseInt(amount || '0'))}</span></div>
          ...
          <div className="w-full space-y-2">
            <button onClick={() => navigate('/records')} className="w-full py-4 rounded-xl text-white font-bold text-base" style={{ background: 'linear-gradient(135deg, #1d4ed8, #3b82f6)' }}>Check Status</button>
