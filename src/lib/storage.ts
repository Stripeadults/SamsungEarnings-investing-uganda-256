...
// FIXED DAILY INCOME - USER GETS ONLY HIS PACKAGE INCOME
export async function runDailyIncomeWithStats(): Promise<{ credited: number; total: number }> {
  ...
  for (const product of products) {
    ...
    // FIXED: Use ONLY product.dailyIncome locked at purchase
    ...
    await supabase.from('samsung_products').update({
      ...
      total_income_earned: product.totalIncomeEarned + lockedIncome,
    }).eq('id', product.id);
    ...
    await addNotification({
      ...
      message: `You earned UGX ${lockedIncome.toLocaleString()} from ${product.packageName}`,
      isRead: false,
    });
