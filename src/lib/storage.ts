...
export async function getUsers(): Promise<User[]> {
  ...
  const step = 1000;
  while(true){
    const { data } = await supabase.from('samsung_users').select('*').order('created_at', { ascending: false }).range(from, from+step-1);
    if(!data || data.length===0) break;
    all = all.concat(data);
    if(data.length < step) break;
    from+=step;
  }
  return all.map(r => dbToUser(r as Record<string, unknown>));
}
...
// FIXED: PAGINATION TO GET ALL PRODUCTS
...
async function findUserByRef(ref: string) {
  if (!ref) return null;
  let { data } = await supabase.from('samsung_users').select('*').eq('id', ref).maybeSingle();
  if (data) return data;
  const { data: byCode } = await supabase.from('samsung_users').select('*').eq('referral_code', ref).maybeSingle();
  return byCode;
}

// FIXED: Approval now sets last_income_date and never blocks activation
  ...
  if (oldProd && oldProd.status === 'pending' && p.status === 'active') {
    try {
      ...
      await supabase.from('samsung_notifications').insert({
        user_id: l1Row.id, type: 'referral_bonus', title: 'L1 Commission Received',
      ...
      if (l1Row.referred_by) {
        try {
          ...
          if (l2Row) {
            ...
            await supabase.from('samsung_notifications').insert({
              user_id: l2Row.id, type: 'referral_bonus', title: 'L2 Commission Received',
            ...
            if (l2Row.referred_by) {
              try {
                ...
                if (l3Row) {
                  ...
                  await supabase.from('samsung_notifications').insert({
                    user_id: l3Row.id, type: 'referral_bonus', title: 'L3 Commission Received',
                    message: `You earned UGX ${l3Reward.toLocaleString()} (1%) from ${packageName}`, is_read: false,
                  });
                }
              } catch {}
            }
...
export async function updateWithdrawal(w: Withdrawal): Promise<void> {
  await supabase.from('samsung_withdrawals').update({
    status: w.status, processed_at: w.processedAt,
...
export async function updateRecharge(r: Recharge): Promise<void> {
  await supabase.from('samsung_recharges').update({
    status: r.status, processed_at: r.processedAt,
  }).eq('id', r.id);
}
...
export async function saveWallet(w: Wallet): Promise<void> {
  await supabase.from('samsung_wallets').upsert({
    id: w.id, user_id: w.userId, type: w.type, phone: w.phone, name: w.name,
  });
}
...
export async function createRedeemCode(c: RedeemCode): Promise<void> {
  await supabase.from('samsung_redeem_codes').insert({
    id: c.id, code: c.code, amount: c.amount, expires_at: c.expiresAt, used_by: c.usedBy, is_active: c.isActive,
  });
}
...
export async function addNotification(notif: Omit<Notification, 'id' | 'createdAt'>): Promise<void> {
  await supabase.from('samsung_notifications').insert({
    user_id: notif.userId, type: notif.type, title: notif.title, message: notif.message, is_read: notif.isRead ?? false,
  });
}
...
// FIXED DAILY INCOME - NEVER STOPS
export async function runDailyIncomeWithStats(): Promise<{ credited: number; total: number }> {
  ...
  for (const product of products) {
    try {
      ...
      const { error: uErr } = await supabase.from('samsung_users').update({
        balance: newBalance,
      ...
      const { data: freshProd } = await supabase.from('samsung_products').select('total_income_earned').eq('id', product.id).single();
      const currentEarned = Number(freshProd?.total_income_earned ?? product.totalIncomeEarned);

      await supabase.from('samsung_products').update({
        last_income_date: now.toISOString(),
        total_income_earned: currentEarned + Number(product.dailyIncome),
      }).eq('id', product.id);

      await addNotification({
        userId: product.userId, type: 'daily_income', title: 'Daily Income Received',
        message: `You earned UGX ${product.dailyIncome.toLocaleString()} from ${product.packageName}`, isRead: false,
      });

      credited += 1;
      total += product.dailyIncome;
    } catch (err) {
      console.error("Skip product", product.id, err);
      continue;
    }
  }
